...
def imshow(tensor, renderer=None, xaxis="", yaxis="", **kwargs):
    ...
    fig = px.imshow(
        to_numpy(tensor), color_continuous_midpoint=0.0, labels={"x": xaxis, "y": yaxis}, **kwargs_pre
...
def lines(
    lines_list, x=None, mode="lines", labels=None, xaxis="", yaxis="", title="", log_y=False, hover=None, **kwargs
):
    # Helper function to plot multiple lines
    if isinstance(lines_list, torch.Tensor):
...
def arg_parse_update_cfg(default_cfg):
    """
    Helper function to take in a dictionary of arguments, convert these to command line arguments, look at what was
    passed in, and return an updated dictionary.
    ...
    for key, value in default_cfg.items():
        if isinstance(value, bool):
            # argparse for Booleans is broken
            if value:
...
def load_gemma_pile_lmsys_mixed_tokens():
    try:
        ...
        data = load_dataset(
            "ckkissane/pile-lmsys-mix-1m-tokenized-gemma-2", split="train", cache_dir="/workspace/cache/"
        )
